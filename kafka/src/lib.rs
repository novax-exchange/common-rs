...
pub fn kafka_consumer(grp_brokers: (String, String), topics: Vec<String>) -> Result<Consumer, KafkaError > {
    let (grp_id, brokers) = grp_brokers;
    let cons = KafkaConsumer::stream_consumer(grp_id, vec![brokers], topics);
    cons
}

pub fn kafka_producer(brokers: Vec<String>) -> Result<Producer, KafkaError >  {
